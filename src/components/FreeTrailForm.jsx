  ...
  return (
    !showBox ? <form onSubmit={handleForm} className={`flex ${direction == "row" ? "md:flex-row flex-col md:items-start items-center justify-center md:flex-wrap  gap-4" : "flex-col gap-2"}`}>
        ...
        <div className='flex flex-col gap-2'>
            ...
            <input type="text" placeholder='Name' value={name} onChange={(e)=> setName(e.target.value)} className={`${transparent == "transparent" ? "bg-transparent  text-white" : "bg-white text-black/50 focus:border-blue-800" } ${color == "black" ? "border border-black/30" : "border border-white"} px-3 h-[52px] outline-none rounded text-sm`}/>
        </div>

        <div className='flex flex-col gap-2'>
            {labelEmail && <label className={`${color == "black" ? "text-black/65" : "text-white/65"} text-xs`}>{labelEmail}</label>}
            <input type="text" placeholder='Email' value={email} onChange={(e)=> setEmail(e.target.value)} className={`${transparent == "transparent" ? "bg-transparent  text-white" : "bg-white text-black/50 focus:border-blue-800" } ${color == "black" ? `border ${validation ? "border-red-800" : "border-black/30" }` : "border border-white"} px-3 h-[52px] outline-none rounded text-sm`}/>
        ...
        <div className='flex flex-col gap-2'>
            ...
            <input type="text" placeholder='Company Name' value={companyName} onChange={(e)=> setCompanyName(e.target.value)} className={`${transparent == "transparent" ? "bg-transparent  text-white" : "bg-white text-black/50 focus:border-blue-800"} ${color == "black" ? "border border-black/30" : "border border-white"} px-3 h-[52px] outline-none rounded text-sm`}/>
        </div>
