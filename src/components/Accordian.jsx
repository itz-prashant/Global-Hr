  ...
  return (
    <div className={`bg-white overflow-hidden transition-all duration-700 ${isOpen ? "max-h-96" : "max-h-14"}`}>
        <div className='border border-gray/40 px-5 py-3 flex gap-2 items-center'>
            ...
            <span onClick={handleToggle} className='cursor-pointer text-black/70 text-[17px]'>{data.title}</span>
        </div>
        <div className={`flex flex-col gap-5 py-5 px-10 transition-all duration-700 ${isOpen ? "border border-gray/40" : ""}`}>
            <h6 className='text-center text-3xl text-black/70 font-medium'>{data.title}</h6>
            <p className='text-black/70'>{data.desccription}.</p>
            {data.list && <ul className='list-disc ml-8 text-black/70'>
                <li>{data.list.list1}</li>
                <li>{data.list.list2}</li>
                <li>{data.list.list3}</li>
                <li>{data.list.list4}</li>
            </ul>}
            {data.list && <p className='text-black/70'>{data.list.para1}</p>}
            {data.list && <p className='text-black/70'>{data.list.para2}</p>}
        </div>
